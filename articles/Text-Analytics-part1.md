...
例えば、ここに、カリフォルニアにある会社がどんなことをしているかを明らかにするための‘California Transparency in Supply Chain Act’というデータがあります。これは、会社のサプライチェーンにおいて、違法に労働搾取をしていたりするのを根絶するためのものです。このデータは、[こちら](http://www.eddiebauer.com/static/pdf/California_Transparency_Supply_Chain.pdf
)からダウンロードできます。
...
専門家ではない限り、理解するのは簡単ではないですが、１つづつ説明していきたいと思います。最後にすべてのステップをRstudioなどで再現することもできます。
...
これから、Exploratory独自の関数である、‘do_tokenize’関数を使って、テキストを単語にトークン化します。
...
幸運にも、この問題を解決するための、‘word stem’や‘stemming’と呼ばれるような技術があります。これは、それぞれの単語の語幹に基いて、派生した単語を減らすことができる技術です。例えば、‘play’, ‘plays’, ‘playing’, ‘played’のような一連の単語は、‘stemming’した後だと、全て‘play’として扱われます。Exploratoryの独自関数である、‘stem_word’関数を使って複数の派生語を語幹に戻しましょう。
...
Runボタンを押す前に、ひとつ大切なことを思い出す必要があります。それは、境界についてです。‘do_ngram’関数は、単に隣り合う単語を連結します。しかし、センテンスをまたいで、単語を連結することは意味を成しません。だから、‘n-gram’は、同じセンテンス内だけで行われる必要があります。
